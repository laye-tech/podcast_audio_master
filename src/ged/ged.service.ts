...
export class GedService extends TypeOrmCrudService<Ged> {
    ...
    constructor(
        ...
        this.baseBucketDeleted = this.configService.get("BUCKET_NAME_WITH_FILES_DELETED")
    ...
    async previewDocument(data: Partial<GedDto>, user: any) {
        ...
        try {
            ...
            const exists = await this.minioService.checkFileExists(this.baseBucket, document.url);

            if (!exists) {
                throw new NotFoundException(`Document non trouvé: ${document.url}`);
            }
            const metadata = await this.minioService.getDocumentMetadata(this.baseBucket, document.url);

            const documentStream = await this.minioService.getDocumentStream(this.baseBucket, document.url);
    ...
    async downloadDocument(url: string, uuid: string, user: any) {
        try {
            const metadata = await this.minioService.getDocumentMetadata(this.baseBucket, url);

            const documentStream = await this.minioService.getDocumentStream(this.baseBucket, url);
    ...
    async saveDocumentUrl(data: GedDto, file: Express.Multer.File, user: any): Promise<Ged> {
        this.logger.log(`🗑️ [DOCUMENT SAVE] Début de l'enregistrement du document.`);
        ...
        try {
            ...
            let savedObjectInBucket = await this.minioService.upload(file, this.baseBucket, data)
    ...
    async updateDocumentWithFile(data: GedDto, file: Express.Multer.File, user: any): Promise<Ged> {
        this.logger.log(`🗑️ [DOCUMENT UPDATE] Début de la modification du document.`);
        ...
        try {
            ...
            await this.minioService.copyObject(this.baseBucketDeleted, this.baseBucket, `${doc.libelle}_archived_${Date.now()}`, sourceObjectNameAndBucket, doc.url)
    ...
    async changeStateDocument(uuid: string, state: string, user: any) {
        this.logger.log(`🗑️ [DOCUMENT UPDATE] Début de la modification du statut du document.`);
        ...
        try {
            ...
            metadata['Modification de Statut '] = this.getFieldChanges(doc.state, state)
    ...
    async updateDocumentWithoutFile(data: GedDto, user: any): Promise<Ged> {
        this.logger.log(`🗑️ [DOCUMENT UPDATE] Début de la modification du document avec fichier joint.`);
    ...
    async deleteDocument(uuid: string, user: any): Promise<void> {
        ...
        const startTime = Date.now();
        this.logger.log(`🗑️ Démarrage de la suppression du document dans la base de données et le cloud pour l'UUID [${uuid}]`);

        try {
            let findDoc = await this.gedRepository.findOneBy({ uuid: uuid })

            if (!findDoc) {
                this.logger.warn(`⚠️ Document avec l'UUID [${uuid}] non trouvé dans la base de données`);
                throw new Error("Le document n'existe pas")
            }

            await this.gedRepository.delete(findDoc.uuid)
                .then(async (el) => {
                    this.logger.log(`✅ Document [${findDoc.libelle}] supprimé de la base de données`);

                    await this.minioService.deleteObject(findDoc.url, this.baseBucket)

                    this.logger.log(`✅ Document [${findDoc.libelle}] supprimé du cloud (URL: ${findDoc.url})`);
    ...
    private async checkExistingDocument(data: Partial<GedDto | State>): Promise<Ged> {
        this.logger.log(`🔍 Vérification de l'existence du document avec l'UUID [${data.uuid}] dans la base de données`);
        ...
        if (!existingDoc) {
            this.logger.warn(`⚠️ Aucune correspondance trouvée pour le document avec l'UUID [${data.uuid}]`);

            throw new ConflictException(`Le document n'existe pas `);
        }

        this.logger.log(`✅ Document trouvé: [${existingDoc.libelle}] avec l'UUID [${existingDoc.uuid}] dans la base de données`);
    ...
    private async checkDuplicatesDocument(data: GedDto): Promise<void> {
        this.logger.log(`🔍 Vérification des doublons pour le document [${data.libelle}] dans la catégorie [${data.categorie}]`);
        const existingDoc = await this.gedRepository.findOneBy({
        ...
        if (existingDoc) {
            this.logger.warn(`⚠️ Document déjà existant : [${data.libelle}] dans la catégorie [${data.categorie}]`);

            throw new ConflictException(`Le document existe déjà pour la categorie: ${data.categorie}`);
        }
        this.logger.log(`✅ Aucun doublon trouvé pour le document [${data.libelle}] dans la catégorie [${data.categorie}]`);
    ...
    private async saveToDatabase(data: GedDto): Promise<Ged> {
        this.logger.log(`Starting database save for document: ${JSON.stringify(data)} 📄`);

        try {
            const savedDoc = await this.gedRepository.save(data) as Ged;
            this.logger.log(`Document saved successfully with ID: ${savedDoc.uuid} ✅`);
            return savedDoc;
        } catch (error) {
            this.logger.error(`Database save failed ❌: ${error.message}`, error.stack);

            throw new Error(
                `Erreur lors de la sauvegarde en base: ${error}`,
            );
        }
    }
    ...
    async updateDocumentBase(data: GedDto, doc: Ged, metadata: Record<string, any>) {
        let tag: Record<string, any>;


        const fieldsToCheck = ['libelle', 'type_document', 'categorie', 'doc_author'];
    ...
    async getAllDoc(params: Partial<GedDto>): Promise<Ged[]> {
        ...
        this.logger.log(
            `-> Fetching All Docs 📂 😀: ${JSON.stringify(params)} `,
        );
