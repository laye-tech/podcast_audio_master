...
export class MinioClientService {
  ...
  public async upload(
    ...
    this.logger.log(
      `🚀 Début de l'upload du fichier : ${file.originalname} 📝`,
    );
    if (
      ![
        'jpeg',
        ...
        'audio/wave',
        'audio/mpeg',
      ].includes(file.mimetype.trim())
    ) {
      this.logger.error(`❌ Type de fichier non autorisé : ${file.mimetype}`);
      throw new HttpException(
        `Ce type de fichier n'est pas autorise: ${file.mimetype}`,
    ...
    const fileName: string = data.categorie
      ? `${data.categorie}/${filename}`
      : filename;
    const fileBuffer = file.buffer;

    this.logger.log(
      `🔄 Téléchargement du fichier vers le bucket [${baseBucket}] dans le dossier [${fileName}]`,
    );
    ...
    this.logger.log(
      `✅ Fichier téléchargé avec succès dans le bucket [${baseBucket}] sous le nom [${fileName}]`,
    );
    this.logger.log(
      `📜 Sauvegarde effectuée avec succès pour le fichier [${file.originalname}] dans le dossier [${fileName}]`,
    );

    return {
      fileName: fileName,
      typeDocument: this.getTypeDocument(file.mimetype.trim()),
    };
  }
  ...
  async createBucketIfNotExists(bucketname: string) {
    ...
    if (!bucketExists) {
      await this.client.makeBucket(
        bucketname,
        this.configService.get('REGION_NAME'),
      );
    }
  }
  async deleteObject(objetName: string, baseBucket: string): Promise<void> {
    this.logger.log(
      `🗑️ Démarrage de la suppression de l'objet [${objetName}] dans le bucket [${baseBucket}]`,
    );
  ...
  async copyObject(
    ...
    baseBucketSource: string,
    fileNameInDestinationBucket: string,
    sourceObjectUrl: string,
    url: string,
  ) {
    this.logger.log(
      `🗑️ Démarrage de la copie de l'objet se trouvant dans le bucket source [${baseBucketSource}] vers le bucket [${destinationBucket}]`,
    );
    ...
    try {
      ...
      await this.client.copyObject(
        destinationBucket,
        fileNameInDestinationBucket,
        sourceObjectUrl,
        null,
      );

      this.logger.log(
        `🗑️ Copie de l'objet se trouvant dans le bucket source [${baseBucketSource}] vers le bucket [${destinationBucket} effectue avec succes]`,
      );
    } catch (error) {
      this.logger.error(
        `Le fichier ${url} n'existe pas dans le bucket ${baseBucketSource}.`,
      );
      throw new Error(
        `Le fichier ${url} n'existe pas dans le bucket ${baseBucketSource}.`,
      );
  ...
  async getDocumentStream(bucketName: string, url: string) {
    this.logger.log(
      `📥 Start fetching document :${url} 📝 from bucket :${bucketName} 📁`,
    );
  ...
  async getDocumentMetadata(bucketName: string, url: string) {
    this.logger.log(
      `Fetching metadata for document: ${url} 📝 in bucket: ${bucketName} 📁`,
    );
  ...
  async checkFileExists(bucketName: string, url: string): Promise<boolean> {
    this.logger.log(
      `🔍 Vérification de l'existence du fichier : ${url} dans le bucket ${bucketName}`,
    );

    try {
      await this.client.statObject(bucketName, url);
      this.logger.log(
        `✅ Le fichier ${url} existe dans le bucket ${bucketName}`,
      );

      return true;
    } catch (error) {
      this.logger.error(
        `❌ Erreur lors de la vérification de l'existence du fichier ${url} dans le bucket ${bucketName} : ${error.message}`,
      );
      return false;
    }
  }
  ...
  async getPresignedUrl(
    ...
    try {
      const url = await this.client.presignedUrl(
        ...
        expiry,
        {
          accessKey: this.configService.get('ACCESS_KEY_MINIO_PLATEFORME'),
          secretKey: this.configService.get('SECRET_KEY_MINIO_PLATEFORME'),
        },
      );
      return url;
    } catch (error) {
